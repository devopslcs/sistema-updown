...
def gerar_pdf_premium(cliente, cnpj, data, blocos, total_calc, total_final, texto_comercial, obs):
    ...
    for i, bloco in enumerate(blocos, 1):
        ...
        if bloco['fotos']:
            ...
            for idx, foto in enumerate(bloco['fotos']):
                try:
                    img = Image.open(foto)
                    temp_f = f"temp_{i}_{idx}.png"
                    img.save(temp_f)
                    pdf.image(temp_f, x=10 + (idx * (largura_f + 5)), y=y_fotos, w=largura_f)
                except: pass
    ...
    # Convertemos o bytearray do fpdf2 em um objeto de bytes puros para o Streamlit
    return bytes(pdf.output())

# --- INTERFACE STREAMLIT ---
st.title("🏗️ UpDown Pro - High Performance")
...
if menu == "Novo Orçamento":
    ...
    for i, bloco in enumerate(st.session_state.blocos):
        with st.container(border=True):
            ...
            bloco['desc_avaria'] = cf2.text_area(f"O que foi identificado?", value=bloco['desc_avaria'], key=f"da_{i}")
            ...
            if ca.button("Add Insumo", key=f"add_{i}"):
                if sel:
            ...
            bloco['valor_mo'] = st.number_input(f"Mão de Obra (R$)", 0.0, value=bloco['valor_mo'], key=f"mo_{i}")
    ...
    txt_com = col_c1.text_area("Condições Comerciais", "Pagamento: 50% entrada / 50% conclusão.", height=150)
    total_final = col_c2.number_input("VALOR TOTAL ACORDADO (R$)", value=float(total_calc))
    ...
    if st.button("🚀 GERAR PROPOSTA PDF", type="primary", use_container_width=True):
        if not cliente:
            ...
            # pdf_bytes agora é garantido como objeto de bytes puro
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name=f"Proposta_UpDown_{cliente}.pdf",
