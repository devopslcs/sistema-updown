...
# Título e Logo
st.title("🏗️ Gerador de Orçamentos - UPDOWN")
st.markdown("---")

# --- 1. BANCO DE DADOS INICIAL (Simulado) ---
# Aqui ficam os itens padrão. Você pode adicionar mais na tela, mas ao reiniciar a página, volta para estes.
if 'materiais' not in st.session_state:
    dados_iniciais = [
        ...
        {"Item": "Impermeabilização Grades de Ar", "Descricao": "Limpeza + Borracha Líquida", "Preco": 2800.00},
        ...
        {"Item": "Borracha Líquida (Balde)", "Descricao": "Balde 20kg", "Preco": 900.00},
...
opcao = st.sidebar.radio("Ir para:", ["Criar Orçamento", "Cadastrar Novo Item"])
...
def gerar_pdf(cliente, cnpj, data, itens, total, obs):
    ...
    for i in itens:
        # Tratamento simples para caracteres especiais no PDF (remove acentos básicos para não quebrar)
...
if opcao == "Cadastrar Novo Item":
    st.subheader("📦 Cadastrar Novo Item no Banco de Dados (Temporário)")
    st.info("Nota: Itens adicionados aqui valem para a sessão atual. Para salvar fixo, altere o código.")
    
    with st.form("form_add"):
        c1, c2 = st.columns([3, 1])
        novo_nome = c1.text_input("Nome do Item / Serviço")
        novo_preco = c2.number_input("Preço Base (R$)", min_value=0.0, value=100.0)
        nova_desc = st.text_input("Descrição Detalhada")
...
# --- TELA 2: CRIAR ORÇAMENTO ---
    ...
    preco_final = c3.number_input("Preço Unitário (R$)", value=float(preco_base))
    ...
    if len(st.session_state.carrinho) > 0:
        ...
        obs = st.text_area("Considerações Finais", value=obs_padrao, height=100)
        
        # Gerar PDF
        if st.button("📄 Baixar PDF do Orçamento"):
            if not cliente:
